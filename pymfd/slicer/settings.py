...
class Settings():
    def __init__(self, header:Header, design:Design, burnin_settings:BurninSettings, default_position_settings:PositionSettings, default_exposure_settings:ExposureSettings):
    ...
    def from_file(cls, filename:str):
        ...
        default_position = PositionSettings(**settings_data["Default Layer Settings"]["Position Settings"])
        default_exposure = ExposureSettings(**settings_data["Default Layer Settings"]["Exposure Settings"])
        return cls(header, design, burnin, default_position, default_exposure)

class Header():
    def __init__(self, schema_version:str, image_directory:str="slices", print_under_vacuum:bool=False):
        self.header = {
...
class ResinType():
    def __init__(self, monomer:list[tuple(str, float)]=[], uv_absorbers:list[tuple(str, float)]=[], initiators:list[tuple(str, float)]=[], additives:list[tuple(str, float)]=[]):
        ...
        if not isinstance(monomer, list) or not all(isinstance(x, tuple) and len(x) == 2 for x in monomer):
            raise ValueError("Monomer must be a list of tuples (name, percentage)")

        if not isinstance(uv_absorbers, list) or not all(isinstance(x, tuple) and len(x) == 2 for x in uv_absorbers):
            raise ValueError("UV absorber must be a list of tuples (name, percentage)")

        if not isinstance(initiators, list) or not all(isinstance(x, tuple) and len(x) == 2 for x in initiators):
            raise ValueError("Initiators must be a list of tuples (name, percentage)")

        if not isinstance(additives, list) or not all(isinstance(x, tuple) and len(x) == 2 for x in additives):
            raise ValueError("Additives must be a list of tuples (name, percentage)")

        if not all(0 <= x[1] <= 100 for x in monomer + uv_absorbers + initiators + additives):
        ...
        if sum(x[1] for x in uv_absorbers + initiators + additives) > 100:
            raise ValueError("UV absorber, initiators, and additives percentages must not exceed 100%")
    ...
    def __str__(self):
        ...
        monomer_str = "_".join(f"{name}-{percentage:.2f}" for name, percentage in self.monomer)
        uv_absorber_str = "_".join(f"{name}-{percentage:.2f}" for name, percentage in self.uv_absorbers)
        initiators_str = "_".join(f"{name}-{percentage:.2f}" for name, percentage in self.initiators)
        if len(self.additives) == 0:
            return f"{monomer_str}__{uv_absorber_str}__{initiators_str}"
        else:
            additives_str = "_".join(f"{name}-{percentage:.2f}" for name, percentage in self.additives)
...
class Resolution():
    def __init__(self, px_size:float=0.0076, px_count:tuple[int, int]=(2560, 1600)):
        ...
        if not isinstance(px_count, tuple) or len(px_count) != 2 or not all(isinstance(x, int) and x > 0 for x in px_count):
...
class Design():
    def __init__(self, user:str="", purpose:str="", description:str="", resin:ResinType=None, printer:Printer=None):
        self.design = {
            "User": user,
            "Purpose": purpose,
...
class PositionSettings():
    def __init__(self, layer_thickness:float=10.0, distance_up:float=1.0, initial_wait:float=0.0, up_speed:float=25.0, up_acceleration:float=50.0, up_wait:float=0.0, down_speed:float=20.0, down_acceleration:float=50.0, force_squeeze:bool=False, squeeze_count:int=0, squeeze_force:float=0.0, squeeze_wait:float=0.0, final_wait:float=0.0):
...
class ExposureSettings():
    def __init__(self, image_file:str="", grayscale_correction:bool=False, image_x_offset:float=0.0, image_y_offset:float=0.0, exposure_time:float=300.0, light_engine:str="Visitech", power_setting:int=100, relative_focus_position:float=0.0, wait_before_exposure:float=0.0, wait_after_exposure:float=0.0):
...
class MembraneSettings():
    def __init__(self, membrane_thickness:float=0.0, exposure_time:float=0.0, dilation:int=0, defocus:float=0.0):
        pass

class EdgeDose():
    def __init__(self, edge_dose_type:str="Errosion", edge_dose:float=0.0, errosion:int=0, dilation:int=0):
        pass

class RoofDose():
    def __init__(self, layers_above:int=0, roof_dose:float=0.0, roof_errosion:int=0, roof_defocus:float=0.0):
        pass
