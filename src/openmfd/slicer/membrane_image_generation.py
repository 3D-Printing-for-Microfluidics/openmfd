    ...
    for i in range(len(slices)):
        ...
        for j in range(prev_image_index + 1, next_image_index):
            ...
            if (
                ...
                bits = np.unpackbits(prev_image, axis=1)
                W = slices[prev_image_index]["image_width"]
                bits = bits[:, :W]
                prev_image = (bits * 255).astype(np.uint8)
            else:
            ...
            if next_image_index >= len(slices):
                ...
                if (
                    ...
                    bits = np.unpackbits(next_image, axis=1)
                    W = slices[next_image_index]["image_width"]
                    bits = bits[:, :W]
                    next_image = (bits * 255).astype(np.uint8)
                else:
            ...
            image_minus_membrane = np.array(image_minus_membrane)
            H, W = image_minus_membrane.shape
            packed_width = (W + 7) // 8
            packed_image = np.zeros((H, packed_width), dtype=np.uint8)
            binary = (image_minus_membrane != 0).astype(np.uint8)
            packed_image = np.packbits(binary, axis=1)
            slices[j]["image_data"] = packed_image
            slices[j]["image_width"] = W
            ...
            dilated_membrane = np.array(dilated_membrane)
            H, W = dilated_membrane.shape
            packed_width = (W + 7) // 8
            packed_image = np.zeros((H, packed_width), dtype=np.uint8)
            binary = (dilated_membrane != 0).astype(np.uint8)
            packed_image = np.packbits(binary, axis=1)
