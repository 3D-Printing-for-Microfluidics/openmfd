...
def generate_secondary_images_from_folders(
    ...
    for _, meta in enumerate(slices):
        ...
        if bulk_image is not None and cv2.bitwise_xor(bulk_image, image).any():
            bulk_image = np.array(bulk_image)
            H, W = bulk_image.shape
            packed_width = (W + 7) // 8
            packed_image = np.zeros((H, packed_width), dtype=np.uint8)
            binary = (bulk_image != 0).astype(np.uint8)
        ...
        if roof_image is not None:
            ...
            roof_image = np.array(roof_image)
            H, W = roof_image.shape
            packed_width = (W + 7) // 8
            packed_image = np.zeros((H, packed_width), dtype=np.uint8)
            binary = (roof_image != 0).astype(np.uint8)
            packed_image = np.packbits(binary, axis=1)
            slice_metadata["secondary_slices"].append(
                {
